...
class Dataset_CRNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform):
        ...
        for i in self.frames:
            # PIL image
            # :03d
            image = Image.open(os.path.join(path, selected_folder, 'frame{:03d}.jpg'.format(i)))
...
class ResCNNEncoder(nn.Module):
    def __init__(self, fc_hidden1, fc_hidden2, fc_hidden3, drop_p, CNN_embed_dim):
...
class DecoderRNN(nn.Module):
    # CNN_embed_dim=300, h_RNN_layers=3, h_RNN=256, h_FC_dim=128, drop_p=0.3, num_classes=50
